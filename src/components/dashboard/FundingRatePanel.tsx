...
import type { FundingRateItem } from '../../hooks/useDashboardData.js';
...
export const FundingRatePanel = React.memo(function FundingRatePanel({ width, height, rates }: FundingRatePanelProps) {
  const visibleRates = rates.slice(0, Math.max(1, height - 3));
  ...
  return (
    ...
    >
      ...
      {rates.length === 0 ? (
        <Text color={colors.muted}>Loading funding rates...</Text>
      ) : (
